#!/bin/bash
#SBATCH --job-name=dino_stats_in1k
...
CFG_DIR="${CFG_DIR:-RAE_ROOT_PLACEHOLDER/configs/stage1/training/ImageNet1k}"
DATA="${DATA:-DATASETS_ROOT_PLACEHOLDER/imagenet_torchvision/data/train}"
STATS_ROOT="${STATS_ROOT:-RAE_ROOT_PLACEHOLDER/assets/stats/ImageNet1k}"
...
case "${SLURM_ARRAY_TASK_ID}" in
  0)
    CONFIG="${CFG_DIR}/DINO_decB_Patch+CLS_prepend.yaml"
    OUT_PT="${OUT_DIR}/dino_cls_tokens.pt"
    ;;
  1)
    CONFIG="${CFG_DIR}/DINO_decB_Patch+Register_prepend.yaml"
    OUT_PT="${OUT_DIR}/dino_register_tokens.pt"
    ;;
  2)
    CONFIG="${CFG_DIR}/DINO_decB_Patch+Register+CLS_prepend.yaml"
