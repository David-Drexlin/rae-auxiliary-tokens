#!/bin/bash
#SBATCH --job-name=mae_stats_in100
...
case "${SLURM_ARRAY_TASK_ID}" in
  0)
    CONFIG="${CFG_DIR}/MAE_decB.yaml"
    OUT_PT="${OUT_DIR}/mae_base.pt"
    ;;
  1)
    CONFIG="${CFG_DIR}/MAE_decB_Patch+CLS_AdaLN.yaml"
    OUT_PT="${OUT_DIR}/mae_cls_pooled.pt"
    ;;
  2)
    CONFIG="${CFG_DIR}/MAE_decB_Patch+CLS_prepend.yaml"
    OUT_PT="${OUT_DIR}/mae_cls_tokens.pt"
    ;;
  *)
