...
LEGACY_TRAIN_EXP_NAME="SIGLIP2_IN100_${MODEL}"
SEEDED_TRAIN_EXP_NAME="SIGLIP2_IN100_${MODEL}_seed${SEED}"
CKPT_DIR="RAE_ROOT_PLACEHOLDER/ckpts/${SEEDED_TRAIN_EXP_NAME}/checkpoints"
if [[ "${SEED}" == "0" && ! -d "${CKPT_DIR}" ]]; then
  CKPT_DIR="RAE_ROOT_PLACEHOLDER/ckpts/${LEGACY_TRAIN_EXP_NAME}/checkpoints"
fi
...
SUBMIT_ARGS=(
  ...
  --export=ALL,CONTAINER="${CONTAINER}",CONFIG="${YAML}",CKPT_DIR="${CKPT_DIR}",DATA=DATASETS_ROOT_PLACEHOLDER/imagenet_torchvision/imagenet100/val,OUTDIR=RAE_ROOT_PLACEHOLDER/assets/recon_samples,SAVE_FOLDER="${SAVE_FOLDER}",REAL_PNG_DIR=RAE_ROOT_PLACEHOLDER/assets/datasets/imagenet100_val_flat,REAL_NPZ=RAE_ROOT_PLACEHOLDER/assets/datasets/imagenet100_val_256.npz,GUIDED_EVAL=HOME_PLACEHOLDER/guided-diffusion/evaluations/evaluator.py,FD_ROOT=HOME_PLACEHOLDER/FD-DINOv2,FD_SCRIPT=HOME_PLACEHOLDER/FD-DINOv2/src/pytorch_fd/fd_score.py,IMAGE_SIZE=256,BATCH_SIZE=16,NUM_WORKERS=8,PRECISION=bf16,NUM_SAMPLES=,TRAIN_EXP_NAME="${TRAIN_EXP_NAME}",METRICS_CSV=RAE_ROOT_PLACEHOLDER/assets/recon_samples/eval_metrics_seeded_siglip2.csv
)
