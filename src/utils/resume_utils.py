...
def find_resume_checkpoint(resume_dir) -> Optional[str]:
    ...
    # Full training checkpoints are named ep-*. Prefer those over decoder-only
    # sidecars such as decoder_ep-*.pt or decoder_ema_ep-*.pt.
    full_checkpoints = [
        path for path in checkpoints
        if os.path.basename(path).startswith("ep-")
    ]
    if len(full_checkpoints) == 0:
        return None

    ep_last = [path for path in full_checkpoints if os.path.basename(path).startswith("ep-last.")]
    if ep_last:
        return sorted(ep_last)[-1]

    numeric_checkpoints = []
    for path in full_checkpoints:
        stem = os.path.splitext(os.path.basename(path))[0]
        suffix = stem[len("ep-"):] if stem.startswith("ep-") else ""
        if suffix.isdigit():
            numeric_checkpoints.append((int(suffix), path))
...
def save_worktree(
    ...
    worktree_path = Path(__file__).resolve().parents[1]
