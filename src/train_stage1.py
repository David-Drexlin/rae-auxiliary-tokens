...
def main():
    ...
    if rae_config is None:
        raise ValueError("Config must define a top-level 'stage_1' section for stage-1 training.")

    data_cfg = OmegaConf.to_container(data_config, resolve=True) if data_config is not None else None
    ...
    gram_weight = float(loss_cfg.get("gram_weight", 0.0))
    ...
    perceptual_cfg = loss_cfg.get("perceptual", {}) if isinstance(loss_cfg, dict) else {}
    perceptual_type = str(perceptual_cfg.get("type", "vgg_lpips")).lower()
    perceptual_input = str(perceptual_cfg.get("input", "11"))  # "01" or "11"
    ...
    aux_dropout_mode = str(rae_config.params.get("aux_dropout_mode", "zero"))
    aux_dropout_prob = float(
        rae_config.params.get(
            "aux_dropout_prob",
            rae_config.params.get("global_cond_dropout_prob", 0.0),
        )
    )
    find_unused_rae = ((aux_dropout_mode == "learned_null") and (aux_dropout_prob > 0.0)) or train_encoder
    if rank == 0 and find_unused_rae:
        reason_bits = []
        if (aux_dropout_mode == "learned_null") and (aux_dropout_prob > 0.0):
            reason_bits.append(
                "learned-null aux dropout activates trainable null parameters only on a subset of batches"
            )
        if train_encoder:
            reason_bits.append(
                "encoder-side trainable modules may not participate on every forward path"
            )
        logger.info(
            "Using DDP find_unused_parameters=True because %s.",
            " and ".join(reason_bits),
        )
    if rank == 0 and train_encoder:
        logger.info(
            "Stage-1 encoder-side trainable modules are enabled by config "
            "(train_encoder=true) while the frozen backbone stays in eval mode."
        )
    ddp_model = DDP(
        rae,
        device_ids=[device.index],
        broadcast_buffers=False,
        find_unused_parameters=find_unused_rae,
    ...
    gram_loss_module = None
    if gram_weight > 0.0:
        gram_cfg = loss_cfg.get("gram", {}) if isinstance(loss_cfg, dict) else {}
        gram_loss_module = GramLoss(
            layer_ids=tuple(gram_cfg.get("layer_ids", [3, 8, 15])),
            layer_weights=tuple(gram_cfg.get("layer_weights", [1.0, 1.0, 1.0])),
            loss_type=str(gram_cfg.get("loss_type", "l1")),
        ).to(device).eval()

    perceptual = None
    if perceptual_weight > 0.0:
        if perceptual_type in ("vgg_lpips", "lpips", "vgg"):
            vgg_cfg = perceptual_cfg.get("vgg_lpips", {})
            perceptual = LPIPS(use_dropout=bool(vgg_cfg.get("use_dropout", True))).to(device).eval()

        elif perceptual_type in ("uni",):
            uni_cfg = perceptual_cfg.get("uni", {})
            perceptual = UNIPerceptualLoss(
                model_id=str(uni_cfg.get("model_id", "hf-hub:MahmoodLab/uni")),
                init_values=float(uni_cfg.get("init_values", 1e-5)),
                dynamic_img_size=bool(uni_cfg.get("dynamic_img_size", True)),
                layers=uni_cfg.get("layers", None),
                drop_tokens=int(uni_cfg.get("drop_tokens", 1)),
                force_fp32=bool(uni_cfg.get("force_fp32", False)),
                amp_dtype=str(uni_cfg.get("amp_dtype", "bf16")),
                normalize_tokens=bool(uni_cfg.get("normalize_tokens", True)),
                assume_minus1_1=bool(uni_cfg.get("assume_minus1_1", True)),
            ).to(device).eval()
        else:
            raise ValueError(f"Unknown gan.loss.perceptual.type: {perceptual_type}")

    #### Opt, Schedl init
    trainable_rae_params = [p for p in ddp_model.module.parameters() if p.requires_grad]
    optimizer, optim_msg = build_optimizer(trainable_rae_params, training_cfg)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if checkpoint_interval > 0 and epoch % checkpoint_interval == 0 and rank == 0:
            ...
            torch.save(ema_model.decoder.state_dict(),
                    f"{checkpoint_dir}/decoder_ema_ep-{epoch:07d}.pt")

            # optional: decoder-only (non-EMA)
            torch.save(ddp_model.module.decoder.state_dict(),
                    f"{checkpoint_dir}/decoder_ep-{epoch:07d}.pt")
        ...
        for step, (images, _) in enumerate(loader):
            ...
            with autocast(**autocast_kwargs):
                ...
                if use_lpips:
                    ...
                    assert perceptual is not None, "perceptual_weight>0 but perceptual module not initialized"
                    if perceptual_input == "01":
                        # UNI can take [0,1] directly; VGG-LPIPS expects [-1,1]
                        if perceptual_type in ("vgg_lpips", "lpips", "vgg"):
                            lpips_loss = perceptual(images * 2.0 - 1.0, recon * 2.0 - 1.0)
                        else:
                            lpips_loss = perceptual(images, recon)
                    else:
                        # "11": use [-1,1]
                        lpips_loss = perceptual(real_normed, recon_normed)

                else:
                    lpips_loss = rec_loss.new_zeros(())

                if gram_weight > 0.0:
                    assert gram_loss_module is not None, "gram_weight>0 but gram_loss_module not initialized"
                    gram_loss = gram_loss_module(images, recon)
                else:
            ...
            if global_step % sample_every == 0 and (rank == 0):
                ...
                with torch.no_grad():
                    ...
                    out_path = samples_dir / f"step-{global_step:09d}.png"
        ...
        if rank == 0 and num_batches > 0:
            print("DEBUG gram_loss =", gram_loss.detach().item())
