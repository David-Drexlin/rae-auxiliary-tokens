    ...
    with torch.inference_mode():
        for _, label in iterator: # don't actually need images at sampling time
            ...
            if use_guidance:
                ...
                y_null = torch.full((n,), int(null_label), device=device, dtype=y.dtype)
                y = torch.cat([y, y_null], dim=0)
            model_kwargs = dict(y=y, **additional_model_kwargs)
            with autocast(**autocast_kwargs):
                samples = final_state_from_trajectory(sample_fn(z, model_fn, **model_kwargs))
                if use_guidance:
                    samples = split_guided_state(samples)
                samples = decode_stage2_state(rae, state_float(samples)).clamp(0,1)
