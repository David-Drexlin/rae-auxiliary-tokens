...
class CrossStreamFusionBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.patch_norm = RMSNorm(hidden_size) if use_rmsnorm else nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.aux_norm = RMSNorm(hidden_size) if use_rmsnorm else nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.patch_to_aux = CrossAttention(
            hidden_size,
            num_heads=num_heads,
            qkv_bias=True,
            qk_norm=use_qknorm,
        ...
        self.aux_to_patch = CrossAttention(
            hidden_size,
            num_heads=num_heads,
            qkv_bias=True,
            qk_norm=use_qknorm,
        ...
        self.patch_gate = nn.Sequential(nn.SiLU(), nn.Linear(hidden_size, hidden_size, bias=True))
        self.aux_gate = nn.Sequential(nn.SiLU(), nn.Linear(hidden_size, hidden_size, bias=True))
    ...
    def forward(
        ...
        patch_delta = self.aux_to_patch(self.patch_norm(patch_tokens), self.aux_norm(aux_tokens))
        aux_delta = self.patch_to_aux(self.aux_norm(aux_tokens), self.patch_norm(patch_tokens))
...
class DualStreamDDT(nn.Module):
    """
    DDT-derived dual-stream Stage-2 model.

    - Patch stream follows the stable DDT patch encoder+decoder structure.
    - Aux stream stays separate and only fuses through cross-attention bridges.
    - Decoder remains patch-only.
    - Supports legacy scalar t or explicit (t_patch, t_aux) in dual-time mode.
    """
    ...
    def __init__(
        ...
        if not self.predict_aux and not self.predict_aux_tokens:
            raise ValueError("DualStreamDDT requires an auxiliary prediction mode.")
        ...
        if isinstance(num_heads, int):
            ...
            self.num_heads = [int(heads[0]), int(heads[1])]
        ...
        if self.predict_aux:
            ...
            self.aux_stream_tokens = self.num_aux_tokens
        ...
        self._last_monitor_stats: Dict[str, float] = {}
    ...
    def forward(self, x, t=None, y=None, meta: Optional[torch.Tensor] = None, aux: Optional[torch.Tensor] = None, **kwargs):
        ...
        monitor_accum: Dict[str, List[float]] = {
        ...
        for patch_block, aux_block, fusion_block in zip(self.patch_encoder_blocks, self.aux_encoder_blocks, self.fusion_blocks):
            ...
            patch_tokens, aux_tokens, block_stats = fusion_block(patch_tokens, aux_tokens, c_patch, c_aux)
        ...
        self._last_monitor_stats = {
            key: torch.stack([value.float() for value in values]).mean()
            for key, values in monitor_accum.items()
        }
        self._last_monitor_stats["aux_output_absmean"] = aux_out.detach().abs().mean()
        self._last_monitor_stats["patch_output_absmean"] = x_img.detach().abs().mean()
