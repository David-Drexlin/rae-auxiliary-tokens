...
class LabelEmbedder(nn.Module):
    ...
    Supports:
      - drop-all (CFG null label) with prob dropout_prob
      - additional per-token drop with prob dropout_prob * 0.5 (only if num_tokens > 1)
    """
    def __init__(self, num_classes, hidden_size, dropout_prob, num_tokens: int = 1):
        ...
        # hardcode: per-conditional (per token) drop = drop/2
        self.token_dropout_prob = 0.5 * self.dropout_prob

        # Always allocate a null embedding slot (index == num_classes)
        self.embedding_table = nn.Embedding(self.num_classes + 1, hidden_size * self.num_tokens)

        # ✅ Backwards-compat alias for your existing init code
        self.embedding_tables = [self.embedding_table]

    def forward(self, labels, train: bool, force_drop_ids=None):
        """
        labels: (B,) int64
        force_drop_ids: optional (B,) mask to force drop-all (CFG unconditional)

        returns:
          if num_tokens == 1: (B, D)
          else:              (B, T, D)   where T=num_tokens
        """
        ...
        null_label = torch.full_like(labels, self.num_classes)  # index == num_classes

        # --- drop ALL conditioning (CFG null label) ---
        drop_all = torch.zeros(B, device=device, dtype=torch.bool)
        if train and self.dropout_prob > 0:
            drop_all |= (torch.rand(B, device=device) < self.dropout_prob)
        if force_drop_ids is not None:
            drop_all |= force_drop_ids.to(device=device).bool()

        labels_all = torch.where(drop_all, null_label, labels)

        emb = self.embedding_table(labels_all)  # (B, T*D)
        D = emb.shape[-1] // self.num_tokens
        emb = emb.view(B, self.num_tokens, D)   # (B, T, D)

        # --- optional per-token drop at drop/2 ---
        if train and self.num_tokens > 1 and self.token_dropout_prob > 0:
            token_drop = (torch.rand(B, self.num_tokens, device=device) < self.token_dropout_prob)
            null_emb = self.embedding_table(null_label).view(B, self.num_tokens, D)
