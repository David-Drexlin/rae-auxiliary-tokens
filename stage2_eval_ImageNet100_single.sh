#!/bin/bash
#SBATCH --job-name=stage2_in100_eval
...
CSV_OUT="${CSV_OUT:-${REPO}/samples_stage2_best80/eval_metrics_stage2_best80.csv}"

FID_REAL_NPZ="${FID_REAL_NPZ:-${REPO}/assets/datasets/imagenet100_val_256.npz}"
...
FD_REAL_IMAGEFOLDER="${FD_REAL_IMAGEFOLDER:-DATASETS_ROOT_PLACEHOLDER/imagenet_torchvision/imagenet100/val}"
...
if [ ! -f "${RUN_DIR}/samples.npz" ]; then
  ...
  apptainer exec --nv \
    ...
    bash -lc "
      ...
      mv '${RUN_DIR}/$(basename "${RUN_DIR}").npz' '${RUN_DIR}/samples.npz'
    "
fi

test -f "${RUN_DIR}/samples.npz" || { echo "[error] missing ${RUN_DIR}/samples.npz after pack"; exit 1; }
