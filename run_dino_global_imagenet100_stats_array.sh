#!/bin/bash
#SBATCH --job-name=dino_global_stats_in100
...
case "${SLURM_ARRAY_TASK_ID}" in
  0)
    CONFIG="${CFG_DIR}/DINO_decB_GlobalCLS.yaml"
    OUT_PT="${OUT_DIR}/dino_global_cls_latent.pt"
    ;;
  1)
    CONFIG="${CFG_DIR}/DINO_decB_GlobalRegister.yaml"
    OUT_PT="${OUT_DIR}/dino_global_register_latent.pt"
    ;;
  2)
    CONFIG="${CFG_DIR}/DINO_decB_GlobalCLSRegister.yaml"
    OUT_PT="${OUT_DIR}/dino_global_cls_register_latent.pt"
    ;;
  *)
