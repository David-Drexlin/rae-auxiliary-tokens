#!/bin/bash
#SBATCH --job-name=dino_variant_stats_in100
...
case "${SLURM_ARRAY_TASK_ID}" in
  0)
    CONFIG="${CFG_DIR}/DINOv2NoReg_decB.yaml"
    OUT_PT="${OUT_DIR}/dino2_noreg_base.pt"
    ;;
  1)
    CONFIG="${CFG_DIR}/DINOv2NoReg_decB_Patch+CLS_prepend.yaml"
    OUT_PT="${OUT_DIR}/dino2_noreg_cls_tokens.pt"
    ;;
  2)
    CONFIG="${CFG_DIR}/DINOv3ViTB16_decB.yaml"
    OUT_PT="${OUT_DIR}/dinov3_vitb16_base.pt"
    ;;
  3)
    CONFIG="${CFG_DIR}/DINOv3ViTB16_decB_Patch+CLS_prepend.yaml"
    OUT_PT="${OUT_DIR}/dinov3_vitb16_cls_tokens.pt"
    ;;
  4)
    CONFIG="${CFG_DIR}/DINOv3ViTB16_decB_Patch+Register_prepend.yaml"
    OUT_PT="${OUT_DIR}/dinov3_vitb16_register_tokens.pt"
    ;;
  5)
    CONFIG="${CFG_DIR}/DINOv3ViTB16_decB_Patch+Register+CLS_prepend.yaml"
    OUT_PT="${OUT_DIR}/dinov3_vitb16_cls_register_tokens.pt"
    ;;
  *)
