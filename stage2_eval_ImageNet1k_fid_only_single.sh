#!/bin/bash
#SBATCH --job-name=stage2_in1k_fid_only
...
CSV_OUT="${CSV_OUT:-${REPO}/samples_stage2_imagenet1k/eval_metrics_stage2_imagenet1k_fid_only.csv}"
...
flock "${LOCK_FILE}" \
  apptainer exec --nv \
    ...
    python3 "${REPO}/src/stage2/eval_stage_1.py" \
      ...
      --fd-real-imagefolder DATASETS_ROOT_PLACEHOLDER/imagenet_torchvision/data/val \
      --device cuda:0 \
      --fd-batch-size 128 \
      --num-workers 8 \
