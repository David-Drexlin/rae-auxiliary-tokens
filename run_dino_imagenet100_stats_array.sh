#!/bin/bash
#SBATCH --job-name=dino_stats_in100
...
case "${SLURM_ARRAY_TASK_ID}" in
  0)
    CONFIG="${CFG_DIR}/DINO_decB.yaml"
    OUT_PT="${OUT_DIR}/dino_base.pt"
    ;;
  1)
    CONFIG="${CFG_DIR}/DINO_decB_Patch+CLS_AdaLN.yaml"
    OUT_PT="${OUT_DIR}/dino_cls_pooled.pt"
    ;;
  2)
    CONFIG="${CFG_DIR}/DINO_decB_Patch+CLS_CA.yaml"
    OUT_PT="${OUT_DIR}/dino_cls_tokens.pt"
    ;;
  3)
    CONFIG="${CFG_DIR}/DINO_decB_Patch+Register_AdaLN.yaml"
    OUT_PT="${OUT_DIR}/dino_register_pooled.pt"
    ;;
  4)
    CONFIG="${CFG_DIR}/DINO_decB_Patch+Register_CA.yaml"
    OUT_PT="${OUT_DIR}/dino_register_tokens.pt"
    ;;
  5)
    CONFIG="${CFG_DIR}/DINO_decB_Patch+Register+CLS_AdaLN.yaml"
    OUT_PT="${OUT_DIR}/dino_cls_register_pooled.pt"
    ;;
  6)
    CONFIG="${CFG_DIR}/DINO_decB_Patch+Register+CLS_CA.yaml"
    OUT_PT="${OUT_DIR}/dino_cls_register_tokens.pt"
    ;;
  *)
...
echo "[JOB] STATS_SUBDIR=${STATS_SUBDIR}"
